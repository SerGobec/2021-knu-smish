...
class Adversary:
    def __init__(self, verbose=True, output='C:/Users/User/Documents/Git/artificial-adversary/Adversary'):
        """
    ...
    def generate(self, texts, links, label, text_sample_rate=1.0, word_sample_rate=0.3, attacks='all', max_attacks=2, random_seed=None, save=True):
        """
        ...
        if save:
            ...
            #print(l)
            if label=='legi':
                #f.writelines("LEGI\t%s %s\n" % ((generated[i][0] for i in range(l)), (links[i] for i in range(l))))
                for i in range(l):
                    f.writelines("LEGI\t" + generated[i][0] + "\n")
            elif label=='smis':
                #f.writelines("SMIS\t%s\n" % generated[i][0] for i in range(len(generated)))
                for i in range(l):
                    f.writelines("SMIS\t" + generated[i][0] + " " + links[i] + "\n")
            elif label=='spam':
                #f.writelines("SPAM\t%s\n" % generated[i][0] for i in range(len(generated)))
                for i in range(l):
                    f.writelines("SPAM\t" + generated[i][0] + " " + links[i] + "\n")
    ...
    def attack(self, texts_original, texts_generated, predict_function, save=True):
        """
        ...
        if save:
            pickle_to_file('res1.txt', misclassifications_single)
            pickle_to_file('res1.txt', misclassifications_group)
